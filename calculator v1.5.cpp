...
enum Token {
    ADD, SUB, MUL, DIV, POW, SQRT, LN, EXP, SIN, COS, TAN, SEC, CSC, COT, ASIN, ACOS, ATAN, LPARA, RPARA, EQUALS, NUM, VAR
};
...
int getPrecedenceToken(Token token) {
    switch(token) {
        ...
        case POW: return 3;
        case SIN: case COS: case TAN: case SEC: case CSC: case COT: case ASIN: case ACOS: case ATAN: case LN: case EXP: case SQRT: return 4;
        default: return 5;
    }
}
...
class Node {
    public:
        ...
        bool isUnary() {
            switch (this -> token) {
                case SIN: case COS: case TAN: case SEC: case CSC: case COT: case ASIN: case ACOS: case ATAN: case LN: case EXP: case SQRT:
                    return true;
                default:
                    return false;
            }
    ...
    public:
        ...
        Expression derivativePointer(Node* node) {
            switch (node -> getToken()) {
                ...
                case SQRT: return (Expression("1") / (Expression("2") * Expression(addNode(Node(SQRT, 0, Expression(node -> getLeftChild()), nullptr))) * derivativePointer(node -> getLeftChild())));
                case LN: return (Expression("1") / derivativePointer(node -> getLeftChild()));
                case EXP: return Expression(addNode(Node(EXP, 0, derivativePointer(node -> getLeftChild()).getRoot(), nullptr)));
                case SIN: return Expression(addNode(Node(COS, 0, derivativePointer(node -> getLeftChild()).getRoot(), nullptr)));
                case COS: return Expression("0") - Expression(addNode(Node(SIN, 0, derivativePointer(node -> getLeftChild()).getRoot(), nullptr)));
                case TAN: return Expression(addNode(Node(SEC, 0, derivativePointer(node -> getLeftChild()).getRoot(), nullptr))) ^ Expression("2");
                case SEC: return Expression(addNode(Node(SEC, 0, derivativePointer(node -> getLeftChild()).getRoot(), nullptr))) * Expression(addNode(Node(TAN, 0, derivativePointer(node -> getLeftChild()).getRoot(), nullptr)));
                case CSC: return (Expression("0") - Expression(addNode(Node(CSC, 0, derivativePointer(node -> getLeftChild()).getRoot(), nullptr)))) * Expression(addNode(Node(COT, 0, derivativePointer(node -> getLeftChild()).getRoot(), nullptr)));
                case COT: return Expression("0") - (Expression(addNode(Node(CSC, 0, derivativePointer(node -> getLeftChild()).getRoot(), nullptr))) ^ Expression("2"));
...
int main() {
    ...
    std::cout << e.getExpressionString() << std::endl;
    return 0;
}
